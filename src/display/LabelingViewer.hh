...
public:
  ...
  /**
     ...
     @param outImage (optional) If non-null, visualized segmentation is stored here
  ...
  viewLabeling(const char* wname, 
    ...
    IplImage* rgbResult = NULL;
    if (visOut == NULL) {
      rgbResult = cvCreateImage(cvGetSize(hsvResult), 8, 3);
    } else {
      rgbResult = visOut;
    }
  ...
  static inline unsigned char getHue(int nLabels, int label) {
    ...
    return (unsigned char)(180 * (double)label / nLabels);
  }
