...
static FeatureGraphExtractor<tFeatureMatrix>* FGraphEx;
static UndirectedFeatureGraph<tFeatureMatrix>* FGraph;
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2) {
    ...
    LearnedWeights = DvecUtils::deserialize(file);
    file.close();
...
void processMouse(int event, int xc, int yc, int flags, void* param) {
  ...
  default: {
    ...
    BinarySubmodularEnergy<tFeatureMatrix> nrg(FGraph);
...
int getXYBlob(int xc, int yc) {
  Blobber *blobber = FGraphEx->getBlobber();
...
int processNewImage(const IplImage *currImage,
  ...
  FGraphEx = new FeatureGraphExtractor<tFeatureMatrix>(currImage);
  ...
  */
  ...
  if (LearnedWeights == NULL) {
    ...
    LearnedWeights = new Dvec(energy.getWeightDim());
  }
...
void processUserInput(int icin) {
  ...
  case 'w': {
    ...
    DvecUtils::serialize(*LearnedWeights, file);
    file.close();
  ...
  case 'l': {			// do learning iterations
    ...
    FGraphEx->getBlobWeights(labelWeights);
    BinarySubmodularEnergy<tFeatureMatrix> energy(FGraph);
    BinarySubmodularObjective<tFeatureMatrix>
      objective(energy, *GCut, 
